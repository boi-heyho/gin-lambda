...
func RequestContext(ctx context.Context) (events.LambdaFunctionURLRequestContext, bool) {
	c, ok := ctx.Value(requestContextKey).(events.LambdaFunctionURLRequestContext)
	return c, ok
}
